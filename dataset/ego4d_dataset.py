...
from dataset.transforms import vertical_flip, YOLOXHSVRandomAug, AlbumentationsAug
...
class ego4dDataset(Dataset):
    ...
    def __init__(self, args, cfg, split, transform=None):
        ...
        self.random_vertical_flip = args.random_vertical_flip
        if args.yolox_hsv_random_aug:
            self.YOLOXHSVRandomAug = YOLOXHSVRandomAug()
        else: 
            self.YOLOXHSVRandomAug = None
        if args.albumentation_aug:
    ...
    def __getitem__(self, idx):
        ...
        # Apply transform for training
        if self.split in ["train"]:
            if self.YOLOXHSVRandomAug is not None:
                input = self.YOLOXHSVRandomAug(input)

            if self.albumentation_aug is not None:
                input = self.albumentation_aug(input)

            if self.random_vertical_flip:
                input, curr_3d_kpts_cam = vertical_flip(input, prob=0.5, keypoints=curr_3d_kpts_cam)
