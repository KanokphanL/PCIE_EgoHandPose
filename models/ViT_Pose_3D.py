...
class ViT_Pose_3D(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.embed_dims = kwargs["BACKBONE"]["EMBED_DIMS"]
        ...
        backbone=dict(
            ...
            img_size=(self.img_H, self.img_W), #self.img_H
            patch_size=16,
            # drop_rate=0.1,
            drop_path_rate=kwargs['BACKBONE']['DROP_PATH_RATE'], #0.1,
            out_type='featmap',
            init_cfg=[
                dict(
                    type='Kaiming',
    ...
    def load_pretrained_weights(self, checkpoint):
        ...
        for k, v in state_dict.items():
            ...
            # if list(model_dict.keys())[0].startswith("backbone.") and (not k.startswith("backbone.")):
            #     k = "backbone." + k
